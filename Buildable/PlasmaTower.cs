...
public class PlasmaTower : BaseTower
{
    ...
    private void Shoot()
    {
        ...
        {
            PlasmaProjectile projectile = (PlasmaProjectile) TakeProjectile();
            projectile.transform.position = shotStartPlaces[0].position;
            projectile.scalingFactor = CalculateScalingFactor();
            projectile.effects[ProjectileEffectsNames.EngineOverheat].percent = CalculateEngineOverheatChance();
    ...
    {
        return 1 * rightUpgradeLevel;
    }

    private int CalculateEngineOverheatChance()
    {
